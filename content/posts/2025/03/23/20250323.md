+++
title = "Automate a developer workflow in Emacs"
author = ["Pieter Swinkels"]
date = 2025-03-23
slug = "automate-a-developer-workflow-in-emacs"
tags = ["emacs"]
draft = false
+++
...
for plots and HTML files for tables. The static website generator [Material for
MkDocs](https://squidfunk.github.io/mkdocs-material/), MkDocs for short, builds a static website from these files.
...
## Manage an external process in Emacs Lisp {#manage-an-external-process-in-emacs-lisp}
...
[start-process](https://www.gnu.org/software/emacs/manual/html_node/elisp/Asynchronous-Processes.html#index-start_002dprocess) command and after some trial &amp; error, I had the following
...
## (Re)start the MkDocs server after a compilation {#re--start-the-mkdocs-server-after-a-compilation}
...
_asynchronously_. So the snippet above would start the server while the build
...
Unfortunately, these hook functions are called after _every_ compilation that is
...
describes how the compilation finished. I found [a blog post](https://www.peterstuart.org/posts/2023-06-04-compilation-notifications/) for a hook function
